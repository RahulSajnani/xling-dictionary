...
class XLingualTrainDataset_baseline_lstm(Dataset):
    ...
    def preprocessing_data(self, phrase, src_lang, src = True):

        tokens = []

        if src:
            if src_lang != "en":
                tokens = self.indic_tokenizer(phrase, src_lang)
            else:
                tokens = self.en_tokenizer(phrase)
    ...
    def __getitem__(self, idx):
        ...
        tokens = self.preprocessing_data(self.phrases[i], self.src_lang[i], src=True)
